    ...
    <div className="space-y-6">
      ...
      {isImportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
          <div className="bg-[#18181f] border border-cyan-500/40 rounded-2xl max-w-3xl w-full p-6 shadow-2xl space-y-5 my-8">
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-white">Import OCTG Inventory from Excel / CSV</h3>
                  <p className="text-xs text-gray-400">Automated column parsing, API Spec 5CT validation, and duplicate detection</p>
                </div>
              </div>
            ...
            <div className="bg-black/30 border border-white/10 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="flex items-center space-x-3">
                <FileCheck className="w-5 h-5 text-emerald-400" />
                <div>
                  <div className="text-xs font-bold text-white">Need the official OCTG import format?</div>
                  <div className="text-[11px] text-gray-400">Download the pre-structured Excel template with sample Casing & Drill Pipe entries.</div>
                </div>
              </div>
              <button
                onClick={handleDownloadSampleTemplate}
                className="px-3.5 py-1.5 rounded-lg text-xs font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-500/30 transition flex items-center space-x-1.5 whitespace-nowrap"
              >
            ...
            <div className="border-2 border-dashed border-white/20 rounded-2xl p-6 text-center hover:border-cyan-500/60 transition bg-black/20">
              <Upload className="w-10 h-10 text-cyan-400 mx-auto mb-2 opacity-80" />
              <p className="text-xs font-semibold text-white">
                Drag and drop your .xlsx, .xls, or .csv file here, or browse
              </p>
              <p className="text-[11px] text-gray-500 mt-1">
                Supports all standard tubular parameters: Tag, OD, Weight, Grade, Connection, Length, S/N, Heat #
              </p>
              
              <label className="mt-4 inline-block px-4 py-2 rounded-xl text-xs font-bold bg-cyan-500 text-black hover:bg-cyan-400 transition cursor-pointer shadow-lg">
            ...
            {importWarnings.length > 0 && (
              <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-xs text-amber-300 space-y-1 max-h-32 overflow-y-auto">
                <div className="font-bold flex items-center space-x-1">
                  <AlertTriangle className="w-3.5 h-3.5" />
                  <span>Warnings Detected ({importWarnings.length}):</span>
                </div>
                {importWarnings.map((w, idx) => (
                  <div key={idx} className="text-[11px] opacity-90">• {w}</div>
                ))}
              </div>
            )}
            ...
            {importPreviewData.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs text-gray-400">
                  <span className="font-bold text-white">Parsed Preview ({importPreviewData.length} records ready to import):</span>
                </div>

                <div className="max-h-56 overflow-y-auto border border-white/10 rounded-xl bg-black/40">
                  <table className="w-full text-left text-[11px] border-collapse">
                    <thead className="bg-white/5 text-gray-400 sticky top-0">
                      <tr>
                        ...
                        <th className="p-2">Specs (OD / Wt / Grade)</th>
                        <th className="p-2">Length</th>
                        <th className="p-2">Serial / Heat</th>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {importPreviewData.map((item, idx) => (
                        <tr key={idx} className="hover:bg-white/5">
                          <td className="p-2 font-mono font-bold text-cyan-300">{item.tagNumber}</td>
                          <td className="p-2 text-gray-200">{item.name}</td>
                          <td className="p-2 text-gray-400">{item.category}</td>
                          <td className="p-2 text-gray-300">{item.outerDiameter} • {item.weightLbFt} • {item.grade}</td>
                          <td className="p-2 font-mono text-white">{item.lengthFt} ft</td>
                          <td className="p-2 font-mono text-gray-400">SN: {item.serialNumber}</td>
                          <td className="p-2 text-gray-300">{item.currentLocation}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="flex items-center justify-end space-x-3 pt-3 border-t border-white/10">
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-xl text-xs font-bold transition flex items-center space-x-2 shadow-lg ${
                  importPreviewData.length > 0
                    ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-black hover:from-emerald-400 hover:to-teal-400 cursor-pointer active:scale-95'
                    : 'bg-white/10 text-gray-500 cursor-not-allowed'
                }`}
              >
                <CheckCircle2 className="w-4 h-4" />
                <span>Commit {importPreviewData.length} Records to Database</span>
              </button>
            </div>
